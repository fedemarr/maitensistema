...
function saldoDeTipo(
  ...
  // Activo y resultados negativos son deudoras; el resto acreedoras.
...
/** Cuentas clave para el hub de Finanzas (Caja, Banco, Deudores, Proveedores…). */
...
export async function balanceComprobacion() {
  ...
  for (const c of porCuenta) {
    if (c.activo)
      porTipo[c.tipo] += saldoDeTipo(c.tipo, c.totalDebe, c.totalHaber);
  }
...
export async function estadoResultados(desde: string, hasta: string) {
  ...
  const cuentas: ResultadoCuenta[] = rows.map((r) => {
    ...
    return { cuentaId: r.cuentaId, codigo: r.codigo, nombre: r.nombre, tipo, debe, haber, neto };
  });
...
export async function getAsiento(id: string) {
  const asiento = await db.query.asientos.findFirst({
    ...
    with: {
      movimiento: { columns: { id: true, tipo: true, fecha: true, observaciones: true } },
      compra: { columns: { id: true, fecha: true, total: true } },
      orden: { columns: { id: true } },
      ccMovimiento: { columns: { id: true, concepto: true } },
  ...
  const totalDebe = asiento.lineas.reduce((a, l) => a + Number(l.debe), 0);
  const totalHaber = asiento.lineas.reduce((a, l) => a + Number(l.haber), 0);
